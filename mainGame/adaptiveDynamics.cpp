/*
    ...
    with neighborhodd list randomly assigned at first generation and random starting point
    in that list at each generation.
    ...
    INPUTS
        Read from terminal:
            ...
            recordInt:  How often the generational trait info is recorder to outfile. 
            pTraitFile: File of doubles (length: popsize*genMax/recordInt) which contains
                            the evolution of production trait over generations.
            nsFile:     File of doubles (length: popsize*genMax/recordInt) which contains
                            the evolution of neighborhood size over generations.
        Read from input file:
            nMin:           Minimum neighborhood size.
            nMax:           Maximum neighborhood size---unused, set to equal popsize.
            ...
            N0:             The initial neighborhood size for the population
...
int n, double p, double meanProductionTrait, int verbosity);
...
/*
    ...
    INPUTS
        ...
        N0 - the initial neighborhood size
        stdevN0 - the initial spread in neighborhood size
        P0 - the initial production
        stdevP0 - the initial spread in production
...
/*
    ...
    INPUTS
        ...
        meanProductionTrait - Mean production of the public good in individual's neighborhood
        verbosity - Determines the amount of output to the terminal

    OUTPUTS
        payoff = benefit - cost
*/
double payoff(double sigma, double beta, double alpha, double kappa, double omega, int n,
...
/*
    ...
    INPUTS
        ...
        nsRecord -  The array which records neighborhood size for export
        productionRecord -  The array which records production for export
        recordInt - The parameter which controls how often the traits are recorded
        verbosity - Determines the amount of output to the terminal
...
double mutThreshold, double *nsRecord, double *productionRecord, int recordInt, int verbosity)
{
    ...
    {  
        ...
        for (k=0; k<(popsize); k++)
        {
            //Make sure neighbor isn't individual
            int check = indices[k]; 
            if (check==nb)
                continue;
            else
    ...
    for (int nGen = 0; nGen < genMax-1; nGen++)
    {
        ...
        //printf("\nNow what is being assigned to 7th individual: \n");
        ...
        for (k = 0; k<popsize; k++)
        {
            ...
            //Count through neighbor list, with random starting index
            neighborStart = uniInt(rng);
            for (i = 0; i < individualNS-1; i++)
            {
                //neighborInd counts the index up from starting value 
                neighborInd = (neighborStart+i)%(popsize-1);
                
                //neighbor converts from (0---->popsize-1) to proper index for gen. and ind.
