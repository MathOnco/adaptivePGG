function plot_adaptiveDynamicsMarkExtinction(productionFile,neighborhoodSizeFile,...
        ...
        if recordExtinction>0
            ...
            for l=j:genAdj-1
                ...
                if abs(centers2(1)-centers2(2)) < (proBranchTolStart/10)
                    ...
                    %Run k means on l-1 gen
                    [~, centers2] = kmeans(productionTraits(l-1,:)',2);
                    
                    %record extinction gen and defector production at extinction
                    extinctionPoint = [(l-1)*recordInt, min(centers2)];
...
if plotProNs>0
    ...
    scatter(x_axis,plotProFlat,...
        2,'filled','MarkerFaceAlpha',1/100,'MarkerFaceColor','k',...
    ...
    if (recordBranchingStart>0 && yes>0)
        plot(branchingPoint(1)/recordInt, branchingPoint(2), 'x', 'Color', ...
            'red','MarkerSize', 20,'LineWidth',2.0)
    end
    ...
    if (recordExtinction>0 && extinct>0)
        plot(extinctionPoint(1)/recordInt, extinctionPoint(2), 'x', 'Color', ...
            'blue', 'MarkerSize', 20,'LineWidth',2.0)
    end
...
if calculatePlotPstar>0
    ...
    scatter(find(isfinite(pstarDefect)).*recordInt,pstarDefect(find(isfinite(pstarDefect))),...
        30,'filled','MarkerFaceAlpha',1/15,'MarkerFaceColor','k',...
        'MarkerEdgeColor','none')
    
    %If we record pstar, tell us where we start recording
    if yes>0
        plot(startIndex,.5, 'x', 'Color', 'blue')
    end
    
    %Plot the theoretical frequency
    plot(find(isfinite(eqFreqDefect)).*recordInt,eqFreqDefect(find(isfinite(eqFreqDefect))),...
    'Color','red','LineWidth',2)
    
    if yes>0
        %Plot Plus 1 sd
        plot(find(isfinite(eqFreqDefect)).*recordInt,eqFreqDefect(find(isfinite(eqFreqDefect)))+sdPstar,...
        'Color',[1 0 0.7],'LineWidth',1)
    ...
    %If turned on and branching occurs, plot a red X where branching occurs
    if (recordBranchingStart>0 && yes>0)
        plot(branchingPoint(1), avgPstar, 'x', 'Color', ...
            'red','MarkerSize', 20,'LineWidth',2.0)
    end
    ...
    %occurs
    if (recordExtinction>0 && extinct>0)
        plot(extinctionPoint(1), avgPstar, 'x', 'Color', ...
            'blue', 'MarkerSize', 20,'LineWidth',2.0)
    end
